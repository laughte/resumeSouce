...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    saveStorageData(state) {
      const itemArray = JSON.stringify(state.personinfo);
      localStorage.setItem('personinfo', itemArray);
    },

    readStoragedata(state) {
      if (localStorage.getItem('personinfo')) {
        const oldarray = localStorage.getItem('personinfo');
        const dataarr = JSON.parse(oldarray);
        if (dataarr.length > 0) {
